...
#' \code{$Env} is the name of the evaluated environment (s).
#' \code{$Response} Variable response obtained for the row corresponding to line and environment.
#' @param NSamples (\code{integer}) Number of groups to the cross-validation.
#' @param fracTesting (0 < numeric <= 1) the fraction of shapes to select.
...
#' Cross-Validation with stratified samples
#'
#' Sample a number of observations to do a cross-validation, this method is similar like random partitions, but is stratified,
#' and could be used like bootstrapping sampling with the \code{replace} parameter as \code{TRUE}.
#'
#' @param DataSet (\code{data.frame}) The object need contain three columns in the Tidy data format:
#' \code{$Line} is the Line or genotype identifier, and the name of this column could change.
