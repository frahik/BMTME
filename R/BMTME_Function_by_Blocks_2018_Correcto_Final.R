...
rmv_f<-function(ps,c,A,x)
{
  ...
  A_ii_inv=V_A_Star%*%V_A_Star_t
   ...
   d_A_Star_Inv=diag(1/d_A_Star)
    ...
    x[Pos_i]    = c(rmvnorm(1,mu_i,A_ii_inv))
...
whichNa <- list()
    whichNa$subjects <- which(apply(FUN = any, X = is.na(Y),
        MARGIN = 1))
...
# eta_0=vec(W)-(vec(u_b1)+vec(u_b2))
    sigmaB.Inv=Cov_Beta_Inv/1E1
    M=sigmaB.Inv+Krone(Re.Inv,tXX)   
    #S_ac=solve(M)
    mu_ac =MatMul(sigmaB.Inv,vec(beta0))+MatMul(Krone(Re.Inv,tX),vec(e))   
    #mu_ac=S_ac%*%mu_ac 
    betav1 =t(MVnormvv(mu_ac,M))
    #t(rmvnorm(1, mean = mu_ac, sigma=S_ac, method="chol"))
...
##### Sample of b1 from a normal distribution ##################################
    ...
    #S_b1=solve(M1)
    mu_b1=MatMul(Krone(Re.Inv,tZ1),vec(e))   
    #mu_b1=S_b1%*%mu_b1 
    #b11 =t(MVnormvv(mu_b1,M1))
    b11 =rmv_f(ps=bs,c= mu_b1,A=M1,x=b1)
    #t(rmvnorm(1, mean = mu_b1, sigma=S_b1, method="chol"))
...
##### Sample of b2 from a normal distribution######################### 
   e=e+u_b2 
    ...
    #S_b2=solve(M2)
    mu_b2 =MatMul(Krone(Re.Inv,tZ2),vec(e))   
    #mu_b2=S_b2%*%mu_b2 
    #b22 =t(MVnormvv(mu_b2,M2))
    b22 =rmv_f(ps=bs,c= mu_b2,A=M2,x=b2)
    #t(rmvnorm(1,mean=mu_b2, sigma=S_b2, method="chol"))
    b2=matrix(b22,ncol=nt,byrow=F)
    u_b2<-MatMul(Z2,b2)
    e=e-u_b2 
           
##### Sample of Environments###########################################
   ME3=matrix(vec(t(b2)),ncol=nI, byrow=F)
...
#########Sample in case of missing values#################
           if ((nNa > 0)) {
            for (j in 1:nNa) {
                ...
                tmp <- sampleY(R =Re, y = Y[subject, ],
                  yHat =yHat[subject, ], e = e[subject, ], missing = missing,observed = observed, traits =nt)
                W[subject, ] <- tmp$y
...
out = list(nIter =nIter,burnIn = burnIn, thin = thin, dfe =ve, Se =Se)
    out$yHat=post_yHat
    ...
    out$varEnv = post_var_b2
    out$SD.varEnv = sqrt(post_var_b2_2 - post_var_b2^2)
