...
#u_b0[tst,]
#YStar1[tst,]=u_b0[tst,]+rmvnorm(nNa,mean=rep(0,nt),sigma=Re, method="chol")
e =(YStar1-u_b0)
EigenRe=eigen(Re)
d_Re=EigenRe$values
...
if ((nNa > 0)) {
W[tst,]=yHat[tst,]+rmvnorm(nNa,mean=rep(0,nt),sigma=Re, method="chol")
...
out = list(nIter =nIter,burnIn = burnIn, thin = thin, dfe =ve, Se =Se)
    out$yHat=post_yHat
 #   rownames(out$yHat) =rownames(y2)
  #  rownames(out$y) =rownames(y2)
...
#    out$post_logLikMean=post_logLik
}
}
out
}
